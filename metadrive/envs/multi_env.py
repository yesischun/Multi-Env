...
DEFAULT_CONFIG = dict(
    ...
    # ===== PG Map Config =====
    map=1,  # int or string: an easy way to fill map_config
    ...
    # ===== Object =====
    # accident may happen on each block with this probability, except multi-exits block
    accident_prob=0.,
...
class MultiEnv(BaseEnv):
    @classmethod
    def default_config(cls) -> Config:
        config = super(MultiEnv, cls).default_config()
        config.update(DEFAULT_CONFIG)
        config.update({'traffic_policy':'IDM',
    ...
    def __init__(self, config: Union[dict, None] = None):
        self.default_config_copy = Config(
            self.default_config(), unchangeable=True)
        super(MultiEnv, self).__init__(config)
    ...
    def _post_process_config(self, config):
        config = super(MultiEnv, self)._post_process_config(config)
    ...
    def done_function(self, vehicle_id: str):
        ...
        if done_info[TerminationState.CRASH_VEHICLE] and self.config["crash_vehicle_done"]:
            ...
            self.logger.debug(
                "Episode ended! Scenario Index: {} Reason: crash vehicle ".format(
                    self.current_seed),
                extra={"log_once": True}
            )
        if done_info[TerminationState.CRASH_OBJECT] and self.config["crash_object_done"]:
            ...
            self.logger.debug(
                "Episode ended! Scenario Index: {} Reason: crash object ".format(
                    self.current_seed),
                extra={"log_once": True}
            )
        if done_info[TerminationState.CRASH_BUILDING]:
            ...
            self.logger.debug(
                "Episode ended! Scenario Index: {} Reason: crash building ".format(
                    self.current_seed),
                extra={"log_once": True}
            )
    ...
    def setup_engine(self):
        super(MultiEnv, self).setup_engine()
        ...
        from metadrive.manager.traffic_manager import DistributionTrafficManager
        self.engine.register_manager("traffic_manager", DistributionTrafficManager())
        if abs(self.config["accident_prob"] - 0) > 1e-2:
            self.engine.register_manager(
                "object_manager", TrafficObjectManager())
...
if __name__ == '__main__':
    ...
    cov_aggressive = GaussianMixtureSampler.build_covariance([0.2, 1.0], 0.5)
    cov_conservative = GaussianMixtureSampler.build_covariance([0.5, 3.0], 0.3)

# traffic_distribution = {"distribution_method": "MultivariateGMM",
#                         # 定义混合高斯参数
#                         "gmm_params": {"weights": [0.3, 0.7], # 30% 激进, 70% 保守 
#                                         # 均值向量 [TTC, Dis]
#                                         "means": [[1.0, 5.0], 
#                                                 [2.5, 15.0]],
#                                         # 协方差矩阵列表
    ...
    config = {"start_seed": 1000, 
              ...
              "traffic_distribution": {
                    ...
                    "gmm_params": {
                        ...
                        "means": [[1.0, 20.0], 
                                  [2.5, 10.0]],
                        "covs": [cov_aggressive, cov_conservative]
                    }}}
